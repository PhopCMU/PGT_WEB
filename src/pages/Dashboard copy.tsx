...
export default function Dashboard() {
  ...
  const sorted = useMemo(() => {
    ...
    if (filter === "open") {
      ...
      filtered = filtered.filter((p) => isSoldOut(p.close_regi));
    }
    ...
    // Sort: open (soonest first) + closed (most recent first)
    const openList = filtered.filter((p) => !isSoldOut(p.close_regi));
    const closedList = filtered.filter((p) => isSoldOut(p.close_regi));

    openList.sort((a, b) => safeToMs(a.close_regi) - safeToMs(b.close_regi));
    closedList.sort((a, b) => safeToMs(b.close_regi) - safeToMs(a.close_regi));
  ...
  const stats = useMemo(
    () => [
      ...
      {
        label: "กำลังเปิดรับ",
        value: projects.filter((p) => isRegiOpen(p.open_regi, p.close_regi))
          .length,
      ...
      {
        label: "จะปิดเร็วนี้",
        value: projects.filter((p) => isClosingSoon(p.close_regi)).length,
                ...
                <div className="flex flex-wrap gap-2">
                  {filter !== "all" && (
                    <span className="inline-flex items-center gap-2 px-3 py-1.5 bg-blue-900/30 text-blue-300 text-xs font-medium rounded-lg border border-blue-800/50">
                      {filter === "open" ? "เปิดรับสมัคร" : "ปิดรับสมัครแล้ว"}
                      <button
